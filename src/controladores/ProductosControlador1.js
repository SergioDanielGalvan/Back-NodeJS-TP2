...
export const getAllProductos = async (req, res) => {
  try {
    ...
    if (!productosConStock) {
      return res
        .status(404)
        .json({ error: "Productos en stock no encontrados" });
    }
...
export const getAllProductosByCategoria = async (req, res) => {
  try {
    ...
    const productosMaestro = await modelMaestro.getAllProducts(
      categoria,
      false,
    );
    if (!productosMaestro) {
...
export const getProductoById = async (req, res) => {
  try {
    ...
    if (!productoEnMaestro) {
      return res
        .status(404)
        .json({ error: "Producto en Maestro no encontrado" });
    }
...
export const getProductoByNombre = async (req, res) => {
  try {
    ...
    const productoenStock = await modelProductos.getProductoByIdProducto(
      productoEnMaestro.idProducto,
    );
    if (!productoenStock) {
      return res.status(404).json({ error: "Producto en stock no encontrado" });
    }
...
export const createProducto = async (req, res) => {
  try {
    ...
    const producto = await modelProductos.createProducto(
      nombre,
      precio,
      categorias,
      stock,
...
export const updateProductoWithStock = async (req, res) => {
  try {
    ...
    const productoActualizado = await modelProductos.updateProductoWithStock(
      id,
      stock,
    );
...
export const updateProductoWithPrecio = async (req, res) => {
  try {
    ...
    const productoActualizado = await modelProductos.updateProductoWithPrecio(
      id,
      precio,
    );
    if (!productoActualizado) {
...
export const crearRegistroCompra = async (req, res) => {
  try {
    ...
    if (!idProducto || !precioCompra || !cantidad) {
      return res
        .status(400)
        .json({ error: "Faltan datos obligatorios para la compra" });
    }
...
export const eliminarProductoByIdLote = async (req, res) => {